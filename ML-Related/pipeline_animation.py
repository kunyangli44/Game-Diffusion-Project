...
class AnimationPipeline(DiffusionPipeline):
    ...
    def prepare_latents(self, batch_size, num_channels_latents, video_length, height, width, dtype, device, generator, latents=None, init_image=None):
        ...
        if init_image is not None:
            # Assume init_image is a PIL Image.
            transform = transforms.Compose([
                transforms.Resize((height, width)),
                transforms.ToTensor(),
                transforms.Normalize([0.5], [0.5])
            ])
            # Convert the image to a tensor and move to the appropriate device.
            image_tensor = transform(init_image).unsqueeze(0).to(device)  # shape: [1, C, H, W]
            # Scale to the range [-1, 1] as expected by the VAE.
            image_tensor = image_tensor * 2 - 1
            # Encode the image with the VAE encoder.
            latent_dist = self.vae.encode(image_tensor).latent_dist
            init_latent = latent_dist.sample() * 0.18215  # scaling factor specific to the VAE
            # If video_length > 1, replicate the latent across the frame dimension.
            init_latent = init_latent.unsqueeze(2).repeat(1, 1, video_length, 1, 1)
            latents = init_latent
        else:
            if latents is None:
